'''
A program made by Shlok Kamat with help from free online references.
...
main_application.title('Brief Text Editor')
...
save_icon = tk.PhotoImage(file = 'icons2/save_.png')
save_as_icon = tk.PhotoImage(file = 'icons2/save.png')
...
## Align Center Button
align_center_icon = tk.PhotoImage(file = 'icons2/align_centre.png')
...
def changed(event = None):
    ...
    if text_editor.edit_modified():
        ...
        characters = len(text_editor.get(1.0, 'end-1c'))   # To not count spaces, add :  >> ).replace(' ','') <<
...
def exit_func(event = None):
    ...
    try:
        if text_changed:
            ...
            if mbox is True:
                if url:
                    content = text_changed.get(1.0, tk.End)
                    with open(url,'w',encoding = 'utf-8') as fw:
                        fw.write(content)
